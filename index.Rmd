---
title: "archivist.github: archivist and GitHub integration"
...
## A simple example of archiving and exploring
...
```{r}
splitTagsRemote(repo = 'Gallery', user = 'MarcinKosinski')
...
getTagsRemote(md5hash = digest::digest(iris.lm.model.smry),
			  tag = "", user = 'MarcinKosinski', repo = 'Gallery' )
```
...
searchInRemoteRepo(pattern = c('class:ggplot', 'class:lm'),
	...
	copyRemoteRepo(md5hashes = ., user = 'pbiecek', repo = 'graphGallery',
				   repoTo = 'Gallery')
showLocalRepo(repoDir = 'Gallery')
```


# Advanced example with [RTCGA](https://github.com/RTCGA) data
...
The data used to produce previous plot came from [*The Cancer Genome Atlas*](http://cancergenome.nih.gov/abouttcga) Study.
